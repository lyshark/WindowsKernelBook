...
实现取进程中模块基址，该功能在`《内核取应用层模块基地址》`中详细介绍过原理，这段代码核心原理如下所示，此处最需要注意的是如果是`32位进程`则我们需要得到`PPEB32 Peb32`结构体，该结构体通常可以直接使用`PsGetProcessWow64Process()`这个内核函数获取到，而如果是`64位进程`则需要将寻找PEB的函数替换为`PsGetProcessPeb()`，其他的枚举细节与上一篇文章中的方法一致。
```c
#include <ntifs.h>
#include <windef.h>
#include "lyshark.h"
...
![](./image/1379525-20230224200542490-1064197181.png)
...
![](./image/1379525-20230224202132245-1128809836.png)