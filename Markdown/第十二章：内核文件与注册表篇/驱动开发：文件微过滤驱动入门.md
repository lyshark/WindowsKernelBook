...
copyright: true
date: '2023-6-19 00:00'
description: >-
  MiniFilter
  微过滤驱动是相对于`SFilter`传统过滤驱动而言的，传统文件过滤驱动相对来说较为复杂，且接口不清晰并不符合快速开发的需求，为了解决复杂的开发问题，微过滤驱动就此诞生，微过滤驱动在编写时更简单，多数`IRP`操作都由过滤管理器`(FilterManager或Fltmgr)`所接管，因为有了兼容层，所以在开发中不需要考虑底层`IRP`如何派发，更无需要考虑兼容性问题，用户只需要编写对应的回调函数处理请求即可，这极大的提高了文件过滤驱动的开发效率。
tags:
  - '《Windows 内核安全编程技术实践》'
  - '第十二章：内核文件与注册表篇'
...
![](./image/1379525-20230226190823919-586000227.png)
